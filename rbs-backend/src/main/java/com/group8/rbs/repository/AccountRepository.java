...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    @Query("SELECT a.accountId FROM Account a WHERE a.email = :email")
    Optional<Integer> findAccountIdByEmail(String email);


    @Query("SELECT a.studentId FROM Account a WHERE a.email = :email")
    Optional<String> findStudentIdByEmail(@Param("email") String email);
}
