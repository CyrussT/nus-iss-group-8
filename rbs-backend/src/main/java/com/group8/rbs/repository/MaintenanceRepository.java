...
public interface MaintenanceRepository extends JpaRepository<MaintenanceSchedule, Long> {
       ...
       /**
        * Find facilities that are under maintenance on a specific date from a list of
        * facility IDs
        * 
       ...
       @Query("SELECT DISTINCT m.facilityId FROM MaintenanceSchedule m " +
                     "WHERE :date BETWEEN m.startDate AND m.endDate")
       List<Long> findFacilitiesUnderMaintenanceOnDateAll(String date);
}